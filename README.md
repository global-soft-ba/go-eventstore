...
![Build](https://img.shields.io/github/actions/workflow/status/global-soft-ba/go-eventstore/ci.yml)
![GoDoc](https://pkg.go.dev/github.com/global-soft-ba/go-eventstore)
...
**Example:**  
You placed an online order on March 1st, but the shipping address was wrong. A support agent wants to see what the system knew at that moment — before you corrected the address later. `AsAt(t)` shows exactly what was stored at the time.
...
**Example:**  
You were supposed to get a pay raise starting June 1st, but HR entered it late. With `AsOf(t)`, payroll can calculate your salary based on the raise as if it had been entered on time.
...
**Example:**  
A sales report is sent to investors on April 2nd, covering March. If new data about March comes in after April 2nd, it shouldn’t affect the original report. Use `AsOfTill(t, r)` to ensure reports always reflect what was known by the report deadline.
